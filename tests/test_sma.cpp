...
using namespace simple_polygon_io::sma;

TEST_CASE("Operator std::map ", "[sma]") {
    SECTION("SmaParams") {
        SmaParams params;
...
TEST_CASE("SmaParams Empty Tests", "[sma]") {
...
TEST_CASE("Json to Result", "[sma]") {
    ...
    "results": {
        "underlying": {
            ...
            "url": "https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/day/1063281600000/1705741285916?limit=235&sort=desc"
        },
        "values": [
            {
                "timestamp": 1705640400000,
                "value": 189.80199999999994
            },
            {
                "timestamp": 1705554000000,
                "value": 189.55539999999993
            },
            {
                "timestamp": 1705467600000,
                "value": 189.31579999999994
            },
            {
                "timestamp": 1705381200000,
                "value": 189.21359999999993
            },
            {
                "timestamp": 1705035600000,
                "value": 189.02039999999994
            },
            {
                "timestamp": 1704949200000,
                "value": 188.71739999999994
            },
            {
                "timestamp": 1704862800000,
                "value": 188.41139999999996
            },
            {
                "timestamp": 1704776400000,
                "value": 188.05199999999994
            },
            {
                "timestamp": 1704690000000,
                "value": 187.68699999999993
            },
            {
                "timestamp": 1704430800000,
                "value": 187.39779999999996
            }
        ]
    },
    "status": "OK",
    "request_id": "34be64f127f25b4afa9f8d58ef989158",
    "next_url": "https://api.polygon.io/v1/indicators/sma/AAPL?cursor="
})"_json;

    JsonResponse response = JsonResponse("AAPL", j);
    REQUIRE(response.status == "OK");
    REQUIRE(response.request_id == "34be64f127f25b4afa9f8d58ef989158");
...
TEST_CASE("Json to Result incomplete ticker", "[sma]") {
    json j = R"({
    "results": {
        "underlying": {
            "aggregates": [
    ...
    REQUIRE(response.status == "OK");
    REQUIRE(response.request_id == "34be64f127f25b4afa9f8d58ef989158");
    REQUIRE(response.result.ohlc.size() == 37);
    for (auto &result: response.result.ohlc) {
        REQUIRE(result.T == "AAPL");
    }
}

TEST_CASE("Json to Result Query", "[sma]") {
    SmaParams params;
    params.set_timespan(Timespan::DAY);
    ...
    JsonResponse response = JsonResponse("AAPL", j);
    response.set_sma_params(params);
    REQUIRE(response.status == "OK");
...
TEST_CASE("Json to Result Query no OHLC", "[sma]") {
    SmaParams params;
    params.set_timespan(Timespan::DAY);
    params.set_window(10);
    params.set_series_type(SeriesType::CLOSE);
...
TEST_CASE("Merge JsonResponse", "[sma]") {
    SmaParams params;
    params.set_timespan(Timespan::DAY);
    params.set_window(10);
    params.set_series_type(SeriesType::CLOSE);
    ...
    params.set_stockticker("AAPL");
    response1.set_sma_params(params);
    params.set_stockticker("A");
    response2.set_sma_params(params);
