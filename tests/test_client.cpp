...
TEST_CASE("Testing PolygonIOManager Aggregates", "[aggregates]") {
    config::PolygonIOConfig config;
    client::PolygonIOClient client(config);

    SECTION("short get aggregates 1") {
        client::AggregatesParams aggregates_params;
        aggregates_params.set_date("2020-10-14");
        aggregates_params.set_stockticker("AAPL");
        aggregates_params.set_adjusted(sma::Adjusted::TRUE);
        aggregates_params.set_timespan(sma::Timespan::DAY);
        auto aggregates = client.get_aggregates(aggregates_params);
        REQUIRE(aggregates.error_message.empty());
        REQUIRE(aggregates.error_found == false);
        REQUIRE(aggregates.count > 0);
        REQUIRE(!aggregates.results.empty());
    }

    SECTION("short get aggregates 2") {
        client::AggregatesParams aggregates_params;
        aggregates_params.set_date("2023-10-16");
        aggregates_params.set_stockticker("A");
        aggregates_params.set_timespan(sma::Timespan::DAY);
        aggregates_params.set_adjusted(sma::Adjusted::FALSE);
        auto aggregates = client.get_aggregates(aggregates_params);
        REQUIRE(aggregates.error_message.empty());
        REQUIRE(aggregates.error_found == false);
        REQUIRE(aggregates.count > 0);
    ...
    SECTION("short get aggregates empty day") {
        client::AggregatesParams aggregates_params;
        aggregates_params.set_stockticker("NVO");
        aggregates_params.set_date("2023-10-14");
        aggregates_params.set_adjusted(sma::Adjusted::FALSE);
        aggregates_params.set_timespan(sma::Timespan::DAY);
        auto aggregates = client.get_aggregates(aggregates_params);
        REQUIRE(aggregates.error_found == true);
        REQUIRE(!aggregates.error_message.empty());
        REQUIRE(aggregates.count == 0);
        REQUIRE(aggregates.results.empty());
        REQUIRE(aggregates.queries("table").empty());
    }
}

TEST_CASE("Testing Merge and queries Aggregates", "[aggregates]") {
    config::PolygonIOConfig config;
    client::PolygonIOClient client(config);
    ...
    SECTION("valid") {
        ...
        aggregates_params.set_adjusted(sma::Adjusted::TRUE);
        aggregates_params.set_timespan(sma::Timespan::DAY);
        auto response = client.get_aggregates(aggregates_params);
        final_response.merge(response);
        REQUIRE(final_response.queries("OHLC").size() == 1);
    }
    ...
    SECTION("fail") {
        ...
        aggregates_params.set_stockticker("AAPL");
        aggregates_params.set_adjusted(sma::Adjusted::TRUE);
        // Timespan is not set, then it will throw an exception
        REQUIRE_THROWS(final_response.merge(client.get_aggregates(aggregates_params)));
    }
}