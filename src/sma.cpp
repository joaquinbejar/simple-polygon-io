...
namespace simple_polygon_io::sma {
    ...
    void SmaParams::set_timestamp(const std::string &timestamp) {
        m_timestamp = timestamp;
    }

    void SmaParams::set_timestamp_gte(const std::string &timestamp_gte) {
        m_timestamp_gte = timestamp_gte;
    }

    void SmaParams::set_timestamp_gt(const std::string &timestamp_gt) {
        m_timestamp_gt = timestamp_gt;
    }

    void SmaParams::set_timestamp_lte(const std::string &timestamp_lte) {
        m_timestamp_lte = timestamp_lte;
    }

    void SmaParams::set_timestamp_lt(const std::string &timestamp_lt) {
        m_timestamp_lt = timestamp_lt;
    }

    void SmaParams::set_stockticker(const std::string &stockticker) {
        m_stockticker = stockticker;
    }

    void SmaParams::set_timespan(Timespan timespan) {
        m_timespan = timespan;
    }

    void SmaParams::set_adjusted(Adjusted adjusted) {
        m_adjusted = adjusted;
    }
    ...
    void SmaParams::set_expand_underlying(ExpandUnderlying expand_underlying) {
        m_expand_underlying = expand_underlying;
    }

    void SmaParams::set_order(Order order) {
        m_order = order;
    }

    void SmaParams::set_limit(size_t limit) {
        m_limit = limit;
    }
    ...
    Query Values::query(const std::string &table) const {
        if (m_stockticker.empty()) {
            throw std::runtime_error("Error parsing simple_polygon_io::sma::Values: ticker was not set");
        }
        if (m_timespan == Timespan::NONE) {
            throw std::runtime_error("Error parsing simple_polygon_io::sma::Values: Params was not set");
        }
    ...
    Result::Result(const std::string &ticker, const json &values) {
        ...
        try {
            ...
            throw std::runtime_error("Error parsing simple_polygon_io::sma::Result values: " + std::string(e.what()));
        }
    }
    ...
    Result::Result(const std::string &ticker, const json &values, const json &aggregates) {
        try {
            ...
            throw std::runtime_error(
                    "Error parsing simple_polygon_io::sma::Result values & aggregates: " + std::string(e.what()));
        }
    }
    ...
    JsonResponse::JsonResponse(std::string ticker, const json &j) : m_ticker(std::move(ticker)) {
        if (m_ticker.empty()) {
            throw std::runtime_error("Error parsing simple_polygon_io::sma::JsonResponse: ticker was not set");
        }
        if (j == nullptr) {
            error_found = true;
        ...
        try {
            ...
            throw std::runtime_error(
                    "Error parsing simple_polygon_io::sma::JsonResponse: " + std::string(e.what()));
        }
    }
    ...
    void JsonResponse::set_sma_params(const SmaParams &sma_params) {
        if (sma_params.get_stockticker().empty())
            throw std::runtime_error(
                    "Error parsing simple_polygon_io::sma::JsonResponse::set_sma_params: ticker was not set");
        for (auto &value: result.values) {
            value.set_sma_params(sma_params);
        }
    }
